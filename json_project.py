...
<g id="outlines">
  ...
  <path id="AL" fill="#D3D3D3" d="M628.5,466.4 l0.6,0.2 1.3,-2.7 1.5,-4.4 2.3,0.6 3.1,6 v1 l-2.7,1.9 2.7,0.3 5.2,-2.5 -0.3,-7.6 -2.5,-1.8 -2,-2 0.4,-4 10.5,-1.5 25.7,-2.9 6.7,-0.6 5.6,0.1 -0.5,-2.2 -1.5,-0.8 -0.9,-1.1 1,-2.6 -0.4,-5.2 -1.6,-4.5 0.8,-5.1 1.7,-4.8 -0.2,-1.7 -1.8,-0.7 -0.5,-3.6 -2.7,-3.4 -2,-6.5 -1.4,-6.7 -1.8,-5 -3.8,-16 -3.5,-7.9 -0.8,-5.6 0.1,-2.2 -9,0.8 -23.4,2.2 -12.2,0.8 -0.2,6.4 0.2,16.7 -0.7,31 -0.3,14.1 2.8,18.8 1.6,14.7z"/>
...
def getTemp(lat,lon):
  response = urllib.request.urlopen('https://api.darksky.net/forecast/a4c4cdbde557c596abd215eaaab6f46f/{},{}'.format(lat,lon))
  responseContent = response.read().decode('utf8')
  temp = json.loads(responseContent)['currently']['temperature']
  print('console.log("lat: {},lon: {}, temp: {}")'.format(lat,lon,temp))
  return temp

def tempToColor(temp):
  int(temp) if type(temp) == int else temp
  if(temp <= 10):
    return 'Blue'
  elif (temp < 31):
    return 'Cyan'
...
for key,value in states_caps.items():
  if(stateCount > 49):
    ...
    stateCount += 1
  curState = key
  curTemp = getTemp(value['lat'], value['long'])
  curColor = tempToColor(curTemp)
  print ("$('#{}').css('fill', '{}')".format(curState,curColor)) 
  time.sleep(0.1)
print('''
});
</script>'''
